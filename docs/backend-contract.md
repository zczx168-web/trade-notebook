...
当前另外提供固定微信收款码的人工收款渠道，配置位于 `manualPayment`。该渠道不调用下述接口，不生成服务器订单或支付状态，不自动开通会员。付款核账和开通安排通过客服线下处理，站内没有人工发放权益的后台。
...
付费内容和权益校验必须由服务端执行。当前本地版全部复盘功能可用，后续付费权益应按正式产品需求实现，不能只靠前端 class 或 localStorage 限制访问。
